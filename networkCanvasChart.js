...
var networkCanvasChart = function () {
  ...
  chart.linkStrength = 0.1;
  chart.zoomTranslate = [80,0];
  chart.collide = false;
  chart.gravity = 0.2;
  chart.useZoomToFit = true;
  // Node's radius
  ...
  */
  ...
  function updateClusters() {
    ...
    //     .strength(chart.useFixedAxis? 0.0000: chart.linkStrength)
    // );
  ...
  chart.zoomToFit = function(svgEle, min, max) {
    // simulation.stop();
    // var zxScale = zoom_transform.rescaleX(xScale),
    ...
    if (min===undefined || max===undefined) {
      filteredNodes = filteredNodes.sort(function (a, b) {
        return d3.ascending(a[chart.xAttr], b[chart.xAttr]);
      });

      min = min || filteredNodes[0];
      max = max || filteredNodes[filteredNodes.length-1];
    }
  ...
  chart.filteredNodes = function() { return filteredNodes; };
  ...
  chart.update = function(selection) {
    selection.each(function(graph) {
      ...
      if (chart.boundaries) {
        simulation
          .force("boundaries", forceBoundary);
      } else {
        simulation
          .force("boundaries", null);
      }
      simulation
      ...
      // Manual Zoom
      if (chart.useZoomToFit) {
      // if (chart.xAttr === "followers_count" || chart.xAttr === "friends_count") {

        // var nytgraphics = filteredNodes.filter(function (d) { return d[chart.nameAttr] === "nytgraphics"; });
        chart.zoomToFit(svgEle.merge(svgEnter));
      ...
      function ticked() {
        ...
        if (chart.drawLinks) {
          if (chart.onlyDrawSelectedLinks) {
            ...
            if (selectedLinks) {
              ...
              context.strokeStyle = chart.linkStrokeStyle;
              simulation.force("link").links().forEach(drawLink);
            }
        ...
        if (selectedNode && selectedLinkedNodes && nonSelectedLinkedNodes) {
          ...
          context.beginPath();
          context.fillStyle = color(selectedNode.party);
